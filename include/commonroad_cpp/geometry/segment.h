...
class Segment {
  ...
  protected:
     ...
     * coordinates.
     */
    std::vector<EigenPolyline> convertRectangleToCartesianCoords(double s_lo_local, double s_hi_local, double l_lo,
                                                                 double l_hi) const;
    ...
    Eigen::RowVectorXd computePseudoDistance(const Eigen::RowVectorXd &lambdas, const Eigen::Matrix2Xd &points);
  ...
  private:
     ...
     */
    Eigen::Vector2d computePseudoNormal(const Eigen::Vector2d &p_lambda, const Eigen::Vector2d &p) const;
     ...
     */
    double computeSignedPseudoDistance(const Eigen::Vector2d &pseudo_normal, const Eigen::Vector2d &p_local) const;
