...
int resample_polyline(const geometry::EigenPolyline &polyline, double step, geometry::EigenPolyline &ret);

int chaikins_corner_cutting(const RowMatrixXd &polyline, int refinements, RowMatrixXd &ret);

int chaikins_corner_cutting(const geometry::EigenPolyline &polyline, int refinements, geometry::EigenPolyline &ret);
 ...
 */
static inline bool intersectionLineLine(const Eigen::Vector2d l1_pt1, const Eigen::Vector2d l1_pt2,
 ...
 */
static inline bool onSegmentGivenColinearPoints(Eigen::Vector2d p, Eigen::Vector2d q, Eigen::Vector2d r) {
    if (std::isgreaterequal(std::max(p[0], r[0]), q[0]) && std::isgreaterequal(q[0], std::min(p[0], r[0])) &&
        std::isgreaterequal(std::max(p[1], r[1]), q[1]) && std::isgreaterequal(q[1], std::min(p[1], r[1]))) {
        return true;
    } else {
        return false;
    }
 ...
 */
static inline Orientation determineOrientation(Eigen::Vector2d p, Eigen::Vector2d q, Eigen::Vector2d r) {
 ...
 */
static inline bool segmentsIntersect(Eigen::Vector2d p1, Eigen::Vector2d q1, Eigen::Vector2d p2, Eigen::Vector2d q2) {
 ...
 * @return True, if segments intersect, otherwise false
 */
static inline bool intersectionSegmentSegment(Eigen::Vector2d l1_pt1, Eigen::Vector2d l1_pt2, Eigen::Vector2d l2_pt1,
                                              Eigen::Vector2d l2_pt2, Eigen::Vector2d &intersection_point) {
 ...
 */
static inline Eigen::Vector2d projectOntoAxes(const Eigen::Matrix2d &matr, const Eigen::Vector2d &point) {
    return matr * point;
}
