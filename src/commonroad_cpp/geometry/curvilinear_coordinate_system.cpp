...
CurvilinearCoordinateSystem::CurvilinearCoordinateSystem(const EigenPolyline &reference_path,
                                                         double default_projection_domain_limit, double eps,
                                                         double eps2) {
    this->length_ = 0.0;
    ...
    if (reference_path.size() < 3) {
        throw std::invalid_argument("<CurvilinearCoordinateSystem> Reference path must have at least 3 "
                                    "points.");
    }
    ...
    this->createSegment(ref_path[0], ref_path[1], ref_path[1] - ref_path[0], ref_path[2] - ref_path[0]);
    for (int i = 1; i < ref_path.size() - 2; i++) {
        this->createSegment(ref_path[i], ref_path[i + 1], ref_path[i + 1] - ref_path[i - 1],
                            ref_path[i + 2] - ref_path[i]);
    }
    int last_idx = ref_path.size() - 1;
    this->createSegment(ref_path[last_idx - 1], ref_path[last_idx], ref_path[last_idx] - ref_path[last_idx - 2],
...
EigenPolyline CurvilinearCoordinateSystem::referencePath() const { return this->reference_path_; }

EigenPolyline CurvilinearCoordinateSystem::referencePathOriginal() const { return this->reference_path_original_; }
...
void CurvilinearCoordinateSystem::setCurvature(std::vector<double> curvature) {
    if (curvature.size() != this->segment_longitudinal_coord_.size()) {
        throw std::invalid_argument("<CurvilinearCoordinateSystem/setCurvature> Curvature values must be "
                                    "given for "
                                    "each segment.");
    }
...
std::tuple<double, double> CurvilinearCoordinateSystem::curvatureRange(double s_min, double s_max) const {
    ...
    if (it_min == this->curvature_.begin()) {
        ...
        throw std::invalid_argument("<CurvilinearCoordinateSystem/curvatureRange> Longitudinal position is "
                                    "outside of "
                                    "coordinate system.");
    } else {
        it_min--;
...
double CurvilinearCoordinateSystem::maximumCurvatureRadius() const {
    if (std::isnan(this->max_curvature_radius_)) {
        throw std::invalid_argument("<CurvilinearCoordinateSystem/getMaximumCurvatureRadius> Maximum "
                                    "curvature radius "
                                    "must be set first.");
    }
...
double CurvilinearCoordinateSystem::minimumCurvatureRadius() const {
    if (std::isnan(this->min_curvature_radius_)) {
        throw std::invalid_argument("<CurvilinearCoordinateSystem/getMinimumCurvatureRadius> Minimum "
                                    "curvature radius "
                                    "must be set first.");
    }
...
Eigen::Vector2d CurvilinearCoordinateSystem::convertToCartesianCoords(double s, double l) const {
    ...
    if (idx < 0) {
        throw std::invalid_argument("<CurvilinearCoordinateSystem/convertToCartesianCoords> Longitudinal "
                                    "coordinate "
                                    "outside of projection domain.");
    }
...
Eigen::Vector2d CurvilinearCoordinateSystem::convertToCurvilinearCoordsAndGetSegmentIdx(double x, double y,
    ...
    if (!is_in_projection_domain) {
        ...
        throw std::invalid_argument("<CurvilinearCoordinateSystem/"
    ...
    std::sort(candidates.begin(), candidates.end(),
              [](const std::pair<Eigen::Vector2d, int> &a, const std::pair<Eigen::Vector2d, int> &b) {
...
EigenPolyline CurvilinearCoordinateSystem::convertListOfPointsToCurvilinearCoords(const EigenPolyline &points,
...
EigenPolyline
CurvilinearCoordinateSystem::convertRectangleToCartesianCoords(double s_lo, double s_hi, double l_lo, double l_hi,
...
void CurvilinearCoordinateSystem::convertPolygonToCurvilinearCoords(
    const EigenPolyline &polygon, std::vector<EigenPolyline> &transformed_polygon) const {
    ...
    this->convertListOfPolygonsToCurvilinearCoordsAndRasterize(polygon_in, groups, 1, 4, transformed_polygons,
...
void CurvilinearCoordinateSystem::convertListOfPolygonsToCurvilinearCoordsAndRasterize(
    ...
    this->determineSubsetsOfMultiPolygonsWithinProjectionDomain(polygons, groups_of_polygons, num_omp_threads,
                                                                clipped_polygon_all, clipped_polygon_groups_all);
...
bool CurvilinearCoordinateSystem::addPointsAtSegmentTransition(
    ...
    for (int i = 1; i < polygon_vertices.size(); i++) {
        ...
        if (segment_idx == previous_segment_idx) {
            ...
            this->interpolatePointsBetweenSegments(polygon_vertices[i - 1], polygon_vertices[i], indices_range,
                                                   curvilinear_polygon);
...
std::list<int> CurvilinearCoordinateSystem::determineIndicesRange(int segment_idx, int previous_segment_idx) const {
...
void CurvilinearCoordinateSystem::interpolatePointsBetweenSegments(const Eigen::Vector2d cartesian_point,
    ...
    for (const auto &k : indices_range) {
        ...
        if (intersect) {
            ...
            Eigen::Vector2d curvilinear_point =
                segment->convertToCurvilinearCoords(intersection_point[0], intersection_point[1]);
...
void CurvilinearCoordinateSystem::rasterizeListOfTransformedPolygonsInProjectionDomain(
    const std::vector<EigenPolyline> &transformed_polygons, const std::set<int> &segment_indices,
    ...
    for (const auto &polygon : transformed_polygons) {
        ...
        transformed_polygons_rasterized.insert(transformed_polygons_rasterized.end(), rectangle_list.begin(),
                                               rectangle_list.end());
    }
}

void CurvilinearCoordinateSystem::rasterizeTransformedPolygonInProjectionDomain(
    const EigenPolyline &transformed_polygon, const std::set<int> &segment_indices,
    ...
    for (it = std::next(segment_indices_extended.begin()); it != segment_indices_extended.end(); ++it) {
        double s_min = std::min(this->segment_longitudinal_coord_[*std::prev(it)], last_maximum_s_coord) - 10e-3;
        ...
        if ((it != segment_indices_extended.end()) && (std::next(it) == segment_indices_extended.end())) {
            copy_if(transformed_polygon.begin(), transformed_polygon.end(), back_inserter(points_in_segment),
                    [s_min](Eigen::Vector2d p) { return std::isgreaterequal(p[0], s_min); });
        } else {
            copy_if(transformed_polygon.begin(), transformed_polygon.end(), back_inserter(points_in_segment),
...
CurvilinearCoordinateSystem::determineSubsetOfPolygonWithinProjectionDomain(const EigenPolyline &polygon) const {
...
std::vector<EigenPolyline>
CurvilinearCoordinateSystem::polygonWithinProjectionDomain(const EigenPolyline &polygon,
...
void CurvilinearCoordinateSystem::createSegment(const Eigen::Vector2d &pt_1, const Eigen::Vector2d &pt_2,
...
std::vector<int> CurvilinearCoordinateSystem::findCandidatePointsInSegment(
    ...
    auto low =
        std::lower_bound(pair_projected_coord_and_id.begin(), pair_projected_coord_and_id.end(), val_min,
                         [](const std::pair<double, int> &lhs, const double rhs) -> bool { return lhs.first < rhs; });

    auto high = std::lower_bound(pair_projected_coord_and_id.begin(), pair_projected_coord_and_id.end(), val_max,
                                 [](std::pair<double, int> lhs, const double rhs) -> bool { return lhs.first < rhs; });
...
void CurvilinearCoordinateSystem::determineCurvilinearCoordinatesAndSegmentIdx(
    ...
    const std::vector<Eigen::RowVectorXd, Eigen::aligned_allocator<Eigen::RowVectorXd>> &l_coord, int num_omp_threads,
        ...
        for (int i = 0; i < groups_of_curvil_points.size(); i++) {
            for (int j = 0; j < groups_of_curvil_points[i].size(); j++) {
                ...
                groups_of_curvil_points[i][j] =
                    std::make_tuple(best_segment, s_coord[best_segment][best_idx], l_coord[best_segment][best_idx]);
            }
        }
    }
}
...
void CurvilinearCoordinateSystem::approximateProjectionDomain(double eps) {
    ...
    for (EigenPolyline::iterator it = projection_domain_border.begin(); it != projection_domain_border.end(); it++) {
...
void CurvilinearCoordinateSystem::approximateCurvilinearProjectionDomain() {
    ...
    if (curvilinear_projection_domain.size() != 1) {
        ...
        throw std::logic_error("<CurvilinearCoordinateSystem/approximateCurvilinearProjectionDomain> "
                               "could not"
    ...
    if (!curvilinear_projection_domain[0].front().isApprox(curvilinear_projection_domain[0].back())) {
        boost::geometry::append(
            ...
            point_type(curvilinear_projection_domain[0].back()[0], curvilinear_projection_domain[0].back()[1]));
    }
}
...
EigenPolyline CurvilinearCoordinateSystem::computeProjectionDomainBorder(double min_radius, double max_radius) {
    ...
    projection_domain_border.insert(projection_domain_border.end(), this->lower_projection_domain_border_.rbegin(),
...
Eigen::VectorXd CurvilinearCoordinateSystem::gradient(const Eigen::VectorXd &input) {
    if (input.size() <= 1)
        return input;
...
Eigen::VectorXd CurvilinearCoordinateSystem::computeCurvature(const geometry::EigenPolyline &polyline) {
    ...
    Eigen::VectorXd curvature = ((x_d.cwiseProduct(y_dd).eval() - x_dd.cwiseProduct(y_d).eval()).array() /
...
#pragma omp for nowait
    ...
    std::vector<std::vector<std::tuple<int, int>>> candidate_segments_of_points(number_of_points_in_all_groups,
                                                                                std::vector<std::tuple<int, int>>(0));
...
#pragma omp for nowait
        for (int segment_index = 0; segment_index < this->segment_list_.size(); segment_index++) {
            ...
            if (n_cand) {
                ...
                Eigen::RowVectorXd s_offset =
                    Eigen::RowVectorXd::Constant(n_cand, this->segment_longitudinal_coord_[segment_index]);
    ...
    this->determineCurvilinearCoordinatesAndSegmentIdx(candidate_segments_of_points, s_results, l_results,
                                                       num_omp_threads, groups_of_curvil_points);
    omp_destroy_lock(&writelock);
