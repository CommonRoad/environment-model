...
#include <commonroad_cpp/geometry/segment.h>

namespace geometry {

Segment::Segment(const Eigen::Vector2d &pt_1, const Eigen::Vector2d &pt_2, const Eigen::Vector2d &t_1,
...
std::vector<EigenPolyline> Segment::convertRectangleToCartesianCoords(double s_lo_local, double s_hi_local, double l_lo,
                                                                      double l_hi) const {
...
Eigen::Vector2d Segment::computePseudoNormal(const Eigen::Vector2d &p_lambda, const Eigen::Vector2d &p) const {
    return p - p_lambda;
}
