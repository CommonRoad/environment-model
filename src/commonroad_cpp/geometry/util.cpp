...
int resample_polyline(const RowMatrixXd &polyline, double step, geometry::EigenPolyline &new_polyline) {
    if (polyline.rows() < 2) {
    ...
    while (current_idx < (polyline.rows() - 1)) {
        if (current_position >= current_length) {
            ...
            new_polyline.push_back(
                ((1 - rel) * polyline.middleRows(current_idx, 1) + rel * polyline.middleRows(current_idx + 1, 1))
                    .eval()
                    .transpose()
                    .eval());
