#include "commonroad_cpp/interfaces/commonroad/protobuf_reader_2023a.h"
#include <utility>

commonroad::CommonRoad ProtobufReader2023a::loadProtobufMessage(const std::string &filePath)
...
void ProtobufReader2023a::initLaneletContainer(ProtobufReader2023a::LaneletContainer &laneletContainer,
...
void ProtobufReader2023a::initTrafficSignContainer(ProtobufReader2023a::TrafficSignContainer &trafficSignContainer,
                                              const commonroad::CommonRoad &commonRoadMsg) {
...
void ProtobufReader2023a::initTrafficLightContainer(ProtobufReader2023a::TrafficLightContainer &trafficLightContainer,
                                               const commonroad::CommonRoad &commonRoadMsg) {
...
void ProtobufReader2023a::initIncomingGroupContainer(IncomingGroupContainer &incomingGroupContainer,
                                           const commonroad::Intersection &intersectionMsg) {
    for (const auto &incomingMsg : intersectionMsg.incomings())
        incomingGroupContainer.emplace(incomingMsg.incoming_id(), std::make_shared<IncomingGroup>());
}

std::shared_ptr<Lanelet> ProtobufReader2023a::getLaneletFromContainer(size_t laneletId,
                                                                      ProtobufReader2023a::LaneletContainer &laneletContainer) {
...
ProtobufReader2023a::getTrafficSignFromContainer(size_t trafficSignId,
                                                 ProtobufReader2023a::TrafficSignContainer &trafficSignContainer) {
...
ProtobufReader2023a::getTrafficLightFromContainer(size_t trafficLightId,
                                                  ProtobufReader2023a::TrafficLightContainer &trafficLightContainer) {
...
ProtobufReader2023a::getIncomingGroupFromContainer(size_t incomingGroupId,
...
ProtobufReader2023a::createCommonRoadFromMessage(const commonroad::CommonRoad &commonRoadMsg) {
    ...
    for (const auto &laneletMsg : commonRoadMsg.lanelets())
        lanelets.push_back(ProtobufReader2023a::createLaneletFromMessage(laneletMsg, laneletContainer, trafficSignContainer,
                                                                    trafficLightContainer));

    std::vector<std::shared_ptr<TrafficSign>> trafficSigns;
    for (const auto &trafficSignMsg : commonRoadMsg.traffic_signs())
        trafficSigns.push_back(ProtobufReader2023a::createTrafficSignFromMessage(trafficSignMsg, trafficSignContainer));

    std::vector<std::shared_ptr<TrafficLight>> trafficLights;
    for (const auto &trafficLightMsg : commonRoadMsg.traffic_lights())
        trafficLights.push_back(ProtobufReader2023a::createTrafficLightFromMessage(trafficLightMsg, trafficLightContainer));

    std::vector<std::shared_ptr<Intersection>> intersections;
    for (const auto &intersectionMsg : commonRoadMsg.intersections())
        intersections.push_back(ProtobufReader2023a::createIntersectionFromMessage(intersectionMsg, laneletContainer));

    std::vector<std::shared_ptr<Obstacle>> obstacles;
    for (const auto &dynamicObstacleMsg : commonRoadMsg.dynamic_obstacles())
        obstacles.push_back(ProtobufReader2023a::createDynamicObstacleFromMessage(dynamicObstacleMsg));

    for (const auto &staticObstacleMsg : commonRoadMsg.static_obstacles())
        obstacles.push_back(ProtobufReader2023a::createStaticObstacleFromMessage(staticObstacleMsg));

    for (const auto &environmentObstacleMsg : commonRoadMsg.environment_obstacles())
        obstacles.push_back(ProtobufReader2023a::createEnvironmentObstacleFromMessage(environmentObstacleMsg));

    for (const auto &phantomObstacleMsg : commonRoadMsg.phantom_obstacles())
        obstacles.push_back(ProtobufReader2023a::createPhantomObstacleFromMessage(phantomObstacleMsg));

    auto [benchmarkId, timeStepSize] =
        ProtobufReader2023a::createScenarioInformationFromMessage(commonRoadMsg.information());
...
ProtobufReader2023a::createScenarioInformationFromMessage(const commonroad::ScenarioInformation &scenarioInformationMsg) {
...
std::shared_ptr<Lanelet> ProtobufReader2023a::createLaneletFromMessage(const commonroad::Lanelet &laneletMsg,
                                                                  LaneletContainer &laneletContainer,
                                                                  TrafficSignContainer &trafficSignContainer,
                                                                  TrafficLightContainer &trafficLightContainer) {
    ...
    auto [leftVertices, leftLineMarking] = ProtobufReader2023a::createBoundFromMessage(laneletMsg.left_bound());
    auto [rightVertices, rightLineMarking] = ProtobufReader2023a::createBoundFromMessage(laneletMsg.right_bound());
    ...
    if (laneletMsg.has_stop_line())
        lanelet->setStopLine(ProtobufReader2023a::createStopLineFromMessage(laneletMsg.stop_line(), trafficSignContainer,
                                                                       trafficLightContainer));
...
ProtobufReader2023a::createBoundFromMessage(const commonroad::Bound &boundMsg) {
    std::vector<vertex> points;
    for (const auto &pointMsg : boundMsg.points())
        points.push_back(ProtobufReader2023a::createPointFromMessage(pointMsg));
...
std::shared_ptr<StopLine> ProtobufReader2023a::createStopLineFromMessage(const commonroad::StopLine &stopLineMsg,
                                                                    TrafficSignContainer &trafficSignContainer,
                                                                    TrafficLightContainer &trafficLightContainer) {
    ...
    for (const auto &point : stopLineMsg.points())
        points.push_back(ProtobufReader2023a::createPointFromMessage(point));
...
std::shared_ptr<TrafficSign> ProtobufReader2023a::createTrafficSignFromMessage(const commonroad::TrafficSign &trafficSignMsg,
                                                                          TrafficSignContainer &trafficSignContainer) {
    ...
    for (const auto &trafficSignElementMsg : trafficSignMsg.traffic_sign_elements())
        trafficSignElements.push_back(ProtobufReader2023a::createTrafficSignElementFromMessage(trafficSignElementMsg));
    trafficSign->setTrafficSignElement(trafficSignElements);

    if (trafficSignMsg.has_position())
        trafficSign->setPosition(ProtobufReader2023a::createPointFromMessage(trafficSignMsg.position()));
...
std::string mapGermanTrafficSignID(const std::string &signName) {
    ...
    if (signName == "MAX_SPEED")
        ...
        return "274";
    throw std::runtime_error{"ProtobufReader2023a::mapGermanTrafficSignID: incomplete, called for " + signName};
}

std::shared_ptr<TrafficSignElement>
ProtobufReader2023a::createTrafficSignElementFromMessage(const commonroad::TrafficSignElement &trafficSignElementMsg) {
...
std::shared_ptr<TrafficLight>
ProtobufReader2023a::createTrafficLightFromMessage(const commonroad::TrafficLight &trafficLightMsg,
                                              TrafficLightContainer &trafficLightContainer) {
    ...
    for (const auto &cycleElementMsg : trafficLightMsg.cycle_elements())
        cycleElements.push_back(ProtobufReader2023a::createCycleElementFromMessage(cycleElementMsg));
    trafficLight->setCycle(cycleElements);

    if (trafficLightMsg.has_position())
        trafficLight->setPosition(ProtobufReader2023a::createPointFromMessage(trafficLightMsg.position()));
...
ProtobufReader2023a::createCycleElementFromMessage(const commonroad::CycleElement &cycleElementMsg) {
...
ProtobufReader2023a::createIntersectionFromMessage(const commonroad::Intersection &intersectionMsg,
                                              LaneletContainer &laneletContainer) {
    std::shared_ptr<Intersection> intersection = std::make_shared<Intersection>();

    intersection->setId(intersectionMsg.intersection_id());

    IncomingGroupContainer incomingContainer;
    initIncomingGroupContainer(incomingContainer, intersectionMsg);

    for (const auto &incomingMsg : intersectionMsg.incomings())
        intersection->addIncomingGroup(
            ProtobufReader2023a::createIncomingFromMessage(incomingMsg, laneletContainer, incomingContainer));
...
std::shared_ptr<IncomingGroup> ProtobufReader2023a::createIncomingFromMessage(const commonroad::Incoming &incomingMsg,
                                                                         LaneletContainer &laneletContainer,
                                               IncomingGroupContainer &incomingContainer) {
    ...
    if (incomingMsg.has_is_left_of()) {
        auto containerIncoming = getIncomingGroupFromContainer(incomingMsg.is_left_of(), incomingContainer);
...
ProtobufReader2023a::createStaticObstacleFromMessage(const commonroad::StaticObstacle &staticObstacleMsg) {
    ...
    staticObstacle->setCurrentState(ProtobufReader2023a::createStateFromMessage(staticObstacleMsg.initial_state()));
    ...
    staticObstacle->setGeoShape(ProtobufReader2023a::createShapeFromMessage(staticObstacleMsg.shape()));

    return staticObstacle;
}

std::shared_ptr<Obstacle>
ProtobufReader2023a::createDynamicObstacleFromMessage(const commonroad::DynamicObstacle &dynamicObstacleMsg) {
    ...
    dynamicObstacle->setGeoShape(ProtobufReader2023a::createShapeFromMessage(dynamicObstacleMsg.shape()));

    dynamicObstacle->setCurrentState(ProtobufReader2023a::createStateFromMessage(dynamicObstacleMsg.initial_state()));

    if (dynamicObstacleMsg.has_initial_signal_state())
        dynamicObstacle->setCurrentSignalState(
            ProtobufReader2023a::createSignalStateFromMessage(dynamicObstacleMsg.initial_signal_state()));
    if (dynamicObstacleMsg.has_trajectory_prediction())
        for (const auto &state :
             ProtobufReader2023a::createTrajectoryPredictionFromMessage(dynamicObstacleMsg.trajectory_prediction()))
    ...
    if (!dynamicObstacleMsg.signal_series().empty())
        for (const auto &state : dynamicObstacleMsg.signal_series())
            dynamicObstacle->appendSignalStateToSeries(ProtobufReader2023a::createSignalStateFromMessage(state));

    return dynamicObstacle;
}

std::shared_ptr<Obstacle>
ProtobufReader2023a::createEnvironmentObstacleFromMessage(const commonroad::EnvironmentObstacle &environmentObstacleMsg) {
    ...
    environmentObstacle->setGeoShape(ProtobufReader2023a::createShapeFromMessage(environmentObstacleMsg.obstacle_shape()));

    return environmentObstacle;
}

std::shared_ptr<Obstacle>
ProtobufReader2023a::createPhantomObstacleFromMessage(const commonroad::PhantomObstacle &phantomObstacleMsg) {
...
ProtobufReader2023a::createTrajectoryPredictionFromMessage(const commonroad::TrajectoryPrediction &trajectoryPredictionMsg) {
    return ProtobufReader2023a::createTrajectoryFromMessage(trajectoryPredictionMsg.trajectory());
}

std::vector<std::shared_ptr<State>>
ProtobufReader2023a::createTrajectoryFromMessage(const commonroad::Trajectory &trajectoryMsg) {
    std::vector<std::shared_ptr<State>> trajectory;

    for (const auto &stateMsg : trajectoryMsg.states())
        trajectory.push_back(ProtobufReader2023a::createStateFromMessage(stateMsg));

    return trajectory;
}

std::shared_ptr<State> ProtobufReader2023a::createStateFromMessage(const commonroad::State &stateMsg) {
    std::shared_ptr<State> state = std::make_shared<State>();

    IntegerExactOrInterval timeStep = ProtobufReader2023a::createIntegerExactOrInterval(stateMsg.time_step());
    if (timeStep.index() == 0)
    ...
    if (stateMsg.has_point()) {
        vertex vertex = ProtobufReader2023a::createPointFromMessage(stateMsg.point());
        state->setXPosition(vertex.x);
        state->setYPosition(vertex.y);
    }

    if (stateMsg.has_orientation()) {
        FloatExactOrInterval orientation = ProtobufReader2023a::createFloatExactOrInterval(stateMsg.orientation());
        if (orientation.index() == 0)
            state->setGlobalOrientation(std::get<double>(orientation));
    }

    if (stateMsg.has_velocity()) {
        FloatExactOrInterval velocity = ProtobufReader2023a::createFloatExactOrInterval(stateMsg.velocity());
        if (velocity.index() == 0)
            state->setVelocity(std::get<double>(velocity));
    }

    if (stateMsg.has_acceleration()) {
        FloatExactOrInterval acceleration = ProtobufReader2023a::createFloatExactOrInterval(stateMsg.acceleration());
...
std::shared_ptr<SignalState> ProtobufReader2023a::createSignalStateFromMessage(const commonroad::SignalState &stateMsg) {
...
vertex ProtobufReader2023a::createPointFromMessage(const commonroad::Point &pointMsg) {
    vertex vertex;
...
std::unique_ptr<Shape> ProtobufReader2023a::createShapeFromMessage(const commonroad::Shape &shapeMsg) {
    std::unique_ptr<Shape> shape;

    if (shapeMsg.has_rectangle())
        shape = ProtobufReader2023a::createRectangleFromMessage(shapeMsg.rectangle());
    else if (shapeMsg.has_circle())
        shape = ProtobufReader2023a::createCircleFromMessage(shapeMsg.circle());

    return shape;
}

std::unique_ptr<Rectangle> ProtobufReader2023a::createRectangleFromMessage(const commonroad::Rectangle &rectangleMsg) {
    ...
    if (rectangleMsg.has_center()) {
        vertex point = ProtobufReader2023a::createPointFromMessage(rectangleMsg.center());
...
std::unique_ptr<Circle> ProtobufReader2023a::createCircleFromMessage(const commonroad::Circle &circleMsg) {
    std::unique_ptr<Circle> circle = std::make_unique<Circle>();

    circle->setRadius(circleMsg.radius());

    if (circleMsg.has_center()) {
        vertex point = ProtobufReader2023a::createPointFromMessage(circleMsg.center());
...
ProtobufReader2023a::IntegerInterval
ProtobufReader2023a::createIntegerIntervalFromMessage(const commonroad::IntegerInterval &integerIntervalMsg) {
    return std::make_pair(integerIntervalMsg.start(), integerIntervalMsg.end());
}

ProtobufReader2023a::FloatInterval
ProtobufReader2023a::createFloatIntervalFromMessage(const commonroad::FloatInterval &floatIntervalMsg) {
    return std::make_pair(floatIntervalMsg.start(), floatIntervalMsg.end());
}

ProtobufReader2023a::IntegerExactOrInterval
ProtobufReader2023a::createIntegerExactOrInterval(const commonroad::IntegerExactOrInterval &integerExactOrIntervalMsg) {
    ...
    if (integerExactOrIntervalMsg.has_exact())
        ...
        integerExactOrInterval = ProtobufReader2023a::createIntegerIntervalFromMessage(integerExactOrIntervalMsg.interval());

    return integerExactOrInterval;
}

ProtobufReader2023a::FloatExactOrInterval
ProtobufReader2023a::createFloatExactOrInterval(const commonroad::FloatExactOrInterval &floatExactOrIntervalMsg) {
    ...
    if (floatExactOrIntervalMsg.has_exact())
        ...
        floatExactOrInterval = ProtobufReader2023a::createFloatIntervalFromMessage(floatExactOrIntervalMsg.interval());
