...
bool InLaneletDrivingDirPredicate::booleanEvaluation(
    ...
    return std::any_of(
        lanelets.begin(), lanelets.end(), [obstacleK, timeStep, theta](const std::shared_ptr<Lanelet> &lanelet) {
            ...
            bool oneWay = std::any_of(usersOneWay.begin(), usersOneWay.end(), [obstacleK](const ObstacleType &user) {
                return (user == obstacleK->getObstacleType()) || (user == ObstacleType::vehicle);
            });
            ...
            bool bidirectional = std::any_of(
                usersBidirectional.begin(), usersBidirectional.end(), [obstacleK](const ObstacleType &user) {
                    return (user == obstacleK->getObstacleType()) || (user == ObstacleType::vehicle);
                });
