...
bool InLaneletDrivingDirPredicate::booleanEvaluation(
    ...
    return std::any_of(
        lanelets.begin(), lanelets.end(), [&, theta](const std::shared_ptr<Lanelet> &lanelet) {
            ...
            bool oneWay = std::any_of(usersOneWay.begin(), usersOneWay.end(), [&](const ObstacleType &user) {
                return user == obstacleK->getObstacleType()
                       or isVehicle.booleanEvaluation(timeStep, world, obstacleK, obstacleP,additionalFunctionParameters);
            });
            ...
            bool bidirectional = std::any_of(
                usersBidirectional.begin(), usersBidirectional.end(), [&](const ObstacleType &user) {
                    return user == obstacleK->getObstacleType()
                           or isVehicle.booleanEvaluation(timeStep, world, obstacleK, obstacleP,additionalFunctionParameters);
                });
