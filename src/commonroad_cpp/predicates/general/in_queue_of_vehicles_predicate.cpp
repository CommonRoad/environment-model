...
bool InQueueOfVehiclesPredicate::booleanEvaluation(
    ...
    for (const auto &obs : world->getObstacles()) {
        if (!obs->timeStepExists(timeStep))
            continue;
        if (obs->getStateByTimeStep(timeStep)->getVelocity() <= parameters.getParam("maxQueueOfVehiclesVelocity") and
    ...
    return num_vehicles >= parameters.getParam("numVehQueueOfVehicles");
}
