...
bool LeftOfBroadLaneMarkingPredicate::booleanEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
    ...
    if (std::any_of(lanelets.begin(), lanelets.end(), [](const std::shared_ptr<Lanelet> &lanelet) {
        ...
        return false;
    auto laneletsRight = obstacle_operations::laneletsRightOfObstacle(timeStep, world->getRoadNetwork(), obstacleK);
    return std::any_of(laneletsRight.begin(), laneletsRight.end(), [](const std::shared_ptr<Lanelet> &lanelet) {
        return lanelet->getLineMarkingLeft() == LineMarking::broad_dashed or
               lanelet->getLineMarkingLeft() == LineMarking::broad_solid;
    });
}
