...
bool DrivesRightmostPredicate::booleanEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
    ...
    if (vehicle_directly_right!= nullptr){
        ...
        for (auto &occLa : occupiedLanelets){
            std::vector<std::shared_ptr<Lane>> lanesOfLanelet = roadNetwork->findLanesByContainedLanelet(occLa->getId());
...
double DrivesRightmostPredicate::robustEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
                                                 const std::shared_ptr<Obstacle> &obstacleK,
                                                 const std::shared_ptr<Obstacle> &obstacleP) {
    throw std::runtime_error("DrivesRightmostPredicate does not support robust evaluation!");
}

Constraint DrivesRightmostPredicate::constraintEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
                                                         const std::shared_ptr<Obstacle> &obstacleK,
                                                         const std::shared_ptr<Obstacle> &obstacleP) {
    throw std::runtime_error("DrivesRightmostPredicate does not support constraint evaluation!");
}
...
std::vector<std::shared_ptr<Obstacle>> DrivesRightmostPredicate::vehiclesRight(size_t timeStep, const std::shared_ptr<World> &world,
                                                                             const std::shared_ptr<Obstacle> &obstacleK) {
...
std::vector<std::shared_ptr<Obstacle>> DrivesRightmostPredicate::vehiclesAdjacent(size_t timeStep,
                                                                                 const std::shared_ptr<World> &world,
                                                                                 const std::shared_ptr<Obstacle> &obstacleK) {
    ...
    for (const auto &obs : otherVehicles) {
        ...
        if (obstacleK->rearS(timeStep) <= obs->rearS(timeStep) and obs->frontS(timeStep) <= obstacleK->frontS(timeStep)){
            vehiclesAdj.push_back(obs);
            continue;
        }
    }
