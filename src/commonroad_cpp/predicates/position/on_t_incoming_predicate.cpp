...
bool OnTIncomingPredicate::booleanEvaluation(
    ...
    const auto currentIncoming_k = intersection_operations::currentIncoming(timeStep, world, obstacleK);
    ...
    for (const auto &lanelet : currentIncoming_k->getIncomingLanelets()) {
        if (lanelet->getId() == t_lanelet->getId())
            return true;
    }
