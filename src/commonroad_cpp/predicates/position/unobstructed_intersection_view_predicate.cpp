...
bool UnobstructedIntersectionViewPredicate::booleanEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
    ...
    for(const auto &inter : obstacle_operations::getIntersections(timeStep, world->getRoadNetwork(), obstacleK)){
        for(const auto &incom : inter->getIncomings()){
            auto newLanes{lanelet_operations::createLanesBySingleLanelets({incom->getIncomingLanelets()}, world->getRoadNetwork(), 50, 0, 1)};
            for(const auto &lane : newLanes){
                ...
                for(size_t idx{lane->getCenterVertices().size()}; idx > 0; --idx){
                    if((dist.back() - dist.at(idx)) > 50 and bg::within(point_type(lane->getCenterVertices().at(idx).x, lane->getCenterVertices().at(idx).y), obstacleK->getFov())){
                            laneContained = true;
                            break;
                            }
                }
...
Constraint UnobstructedIntersectionViewPredicate::constraintEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
                                                         const std::shared_ptr<Obstacle> &obstacleK,
                                                         const std::shared_ptr<Obstacle> &obstacleP) {
