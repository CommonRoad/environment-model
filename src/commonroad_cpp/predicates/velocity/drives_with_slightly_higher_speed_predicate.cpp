...
bool DrivesWithSlightlyHigherSpeedPredicate::booleanEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
    ...
    return 0 < (obstacleK->getStateByTimeStep(timeStep)->getVelocity() - obstacleP->getStateByTimeStep(timeStep)->getVelocity()) < parameters.slightlyHigherSpeedDifference;
}

Constraint DrivesWithSlightlyHigherSpeedPredicate::constraintEvaluation(size_t timeStep, const std::shared_ptr<World> &world,
