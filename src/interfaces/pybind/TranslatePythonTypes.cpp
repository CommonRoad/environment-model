//######################################################################################################################
// DESCRIPTION: Implementation of the conversion functions defined in "TranslatePythonTypes.h"
//######################################################################################################################
...
std::vector<std::shared_ptr<vehicularLanelet>>
TranslatePythonTypes::convertLanelets(const py::handle &py_laneletNetwork) {
    // todo add checks if laneletContainer is unused

    std::vector<std::shared_ptr<vehicularLanelet>> laneletContainer;
    laneletContainer.clear();
    ...
    try {
        ...
        std::cout << "Using traffic signs to determine speed limit." << std::endl;

    } catch (const std::exception &e) {
        usesTrafficSigns = false;
        std::cout << "Not using traffic signs to determine speed limit." << std::endl;
    }
    ...
    for (py::handle py_SingleLanelet : py_lanelets) {
        ...
        double speed_lim;
        if (usesTrafficSigns)
            speed_lim = extractSpeedLimit2020(py_SingleLanelet, speedLimits);
        else
            speed_lim = extractSpeedLimit2018(py_SingleLanelet);
        ...
        std::vector<vertice> left_vertices;
        ...
        std::vector<vertice> right_vertices;
        ...
        std::vector<vertice> center_vertices;
        ...
        // For now it is assumed, that the lanelet is vehicularlanelet (Like in the old interface)
        ...
        std::shared_ptr<vehicularLanelet> tempLanelet =
            std::make_shared<vehicularLanelet>(); // make_shared is faster than (new vehicularLanelet());
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/// \brief This function adds the predecessor and sucessor lanelet pointers to the laneletObjects
///
...
uint8_t TranslatePythonTypes::convertObstacles(const py::list &py_dynamicObstacles,
    ...
    for (py::handle py_singleObstacle : py_dynamicObstacles) {
        ...
        if (tempObstacle->getGeoShape().getType() == "Rectangle") {
            ...
            vertice CenterPoint; // Center point assigned to circular shape
