...
// uint8_t py_registerScenario2018b(const int &ScenarioID, const py::handle &py_laneletNetwork, const py::list
// &py_obstacles);
...
PYBIND11_MODULE(cpp_env_model, m) {
    ...
    m.def("registerScenario", &py_registerScenario, "Add new scenario to C++ environment model", py::arg("scenarioId"),
