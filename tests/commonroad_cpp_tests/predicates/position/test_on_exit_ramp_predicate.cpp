...
// Credits: BMW Car@TUM
//
#include "test_on_exit_ramp_predicate.h"
#include "../utils_predicate_test.h"
#include "commonroad_cpp/obstacle/state.h"

void OnExitRampPredicateTest::SetUp() {
    ...
    auto roadNetwork{utils_predicate_test::create_road_network({LaneletType::exitRamp, LaneletType::interstate})};
    world = std::make_shared<World>(World(0, roadNetwork, {egoVehicle}, {}, 0.1));
}

TEST_F(OnExitRampPredicateTest, BooleanEvaluationObjects) {
...
TEST_F(OnExitRampPredicateTest, RobustEvaluation) {
    EXPECT_THROW(pred.robustEvaluation(0, world, egoVehicle), std::runtime_error);
}

TEST_F(OnExitRampPredicateTest, ConstraintEvaluation) {
