...
void TrafficLightTest::SetUp() {
    ...
    light1 = std::make_shared<TrafficLight>(lightId1, cycle1, offset1, Direction::left, true, pos1);
    ...
    light2->setDirection(Direction::straightRight);
    light2->setOffset(0);
    ...
    light3 = std::make_shared<TrafficLight>(lightId3, cycle3, offset3, Direction::leftStraight, true, pos1);

    size_t lightId4{4};
    vertex pos4{1007, 1008};
    size_t offset4{0};
    std::vector<TrafficLightCycleElement> cycle4{{TrafficLightState::green, 20}, {TrafficLightState::red, 100}};
    light4 = std::make_shared<TrafficLight>(lightId4, cycle4, offset4, Direction::leftRight, true, pos1);
}
...
TEST_F(TrafficLightTest, InitializationComplete) {
    ...
    EXPECT_EQ(light1->getDirection(), Direction::left);
    ...
    EXPECT_EQ(light2->getDirection(), Direction::straightRight);
    ...
    EXPECT_EQ(light3->getDirection(), Direction::leftStraight);
    EXPECT_EQ(light3->getElementAtTime(0).color, TrafficLightState::red_yellow);

    EXPECT_EQ(light4->getId(), 4);
    EXPECT_EQ(light4->getDirection(), Direction::leftRight);
}
