...
// Credits: BMW Car@TUM
//
#include "test_on_main_carriage_way_predicate.h"
#include "../utils_predicate_test.h"
#include "commonroad_cpp/obstacle/state.h"

void OnMainCarriageWayPredicateTest::SetUp() {

    std::shared_ptr<State> stateZeroEgoVehicle = std::make_shared<State>(0, 0, 0, 10, 0, 0, 0, 0, 0);
    std::shared_ptr<State> stateOneEgoVehicle = std::make_shared<State>(1, 10, 3, 10, 0, 0, 0, 10, 0);
    std::shared_ptr<State> stateTwoEgoVehicle = std::make_shared<State>(2, 20, 6, 10, 0, 0, 0, 20, 0);
    std::shared_ptr<State> stateThreeEgoVehicle = std::make_shared<State>(3, 30, 7, 10, 0, 0, 0, 30, 0);
    std::shared_ptr<State> stateFourEgoVehicle = std::make_shared<State>(4, 40, 8, 10, 0, 0, 0, 40, 0);
    std::shared_ptr<State> stateFiveEgoVehicle = std::make_shared<State>(5, 50, 11, 10, 0, 0, 0, 50, 0);
...
TEST_F(OnMainCarriageWayPredicateTest, BooleanEvaluationObjects) {
