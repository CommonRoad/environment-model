...
TEST_F(LaneletOperationsTest, CombineLaneletAndSuccessorsWithSameTypeToLane){
    EXPECT_EQ(combineLaneletAndSuccessorsWithSameTypeToLane(laneletThree,
                                                            LaneletType::mainCarriageWay)->getLanelet().getId(),
              roadNetwork->getLanes().at(0)->getLanelet().getId());
    compareVerticesVector(combineLaneletAndSuccessorsWithSameTypeToLane(laneletThree,
                                                          LaneletType::mainCarriageWay)->
                                                          getLanelet().getCenterVertices(),
                                                          roadNetwork->getLanes().at(0)->
                                                          getLanelet().getCenterVertices());
    EXPECT_EQ(combineLaneletAndSuccessorsWithSameTypeToLane(laneletSeven,
                                                            LaneletType::mainCarriageWay)->
            getLanelet().getId(),10);
    EXPECT_EQ(combineLaneletAndSuccessorsWithSameTypeToLane(laneletThree,
                                                            LaneletType::urban)->
