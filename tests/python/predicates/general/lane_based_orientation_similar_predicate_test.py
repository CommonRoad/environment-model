...
class TestOrientationTowardsPredicate(unittest.TestCase):
    ...
    def test_orientation_towards(self):
        ...
        obstacle_2 = DynamicObstacle(2, ObstacleType.CAR, Rectangle(5, 2),
                                     InitialState(time_step=0, position=np.array([0, 2]), velocity=10,
                                           acceleration=0, orientation=0),
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         CustomState(time_step=1, position=np.array([10, 4.01]), velocity=10, acceleration=0,
                                               orientation=0),
                                         CustomState(time_step=2, position=np.array([20, 2]), velocity=10, acceleration=0,
                                               orientation=0),
                                         CustomState(time_step=3, position=np.array([30, 2]), velocity=10, acceleration=0,
        ...
        # Monitor-Mode
        sol_monitor_mode_1 = crcpp.lane_based_orientation_similar_boolean_evaluation(123, 0, 1, 2)
        sol_monitor_mode_2 = crcpp.lane_based_orientation_similar_boolean_evaluation(123, 1, 1, 2)
        sol_monitor_mode_3 = crcpp.lane_based_orientation_similar_boolean_evaluation(123, 2, 1, 2)
        sol_monitor_mode_4 = crcpp.lane_based_orientation_similar_boolean_evaluation(123, 3, 1, 2)
        sol_monitor_mode_5 = crcpp.lane_based_orientation_similar_boolean_evaluation(123, 1, 1, 3)
