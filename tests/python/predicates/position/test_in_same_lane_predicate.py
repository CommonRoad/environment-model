...
class TestInSameLanePredicate(unittest.TestCase):
    ...
    def test_in_same_lane(self):
        ...
        obstacle_2 = DynamicObstacle(2, ObstacleType.CAR, Rectangle(5, 2),
                                     InitialState(time_step=0, position=np.array([10, 2]), velocity=2,
                                                  acceleration=0, orientation=0),
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         CustomState(time_step=1, position=np.array([20, 2]), velocity=10,
                                                     acceleration=0,
                                                     orientation=0),
                                         CustomState(time_step=2, position=np.array([30, 4]), velocity=10,
                                                     acceleration=0,
                                                     orientation=0),
                                         CustomState(time_step=3, position=np.array([40, 6]), velocity=10,
                                                     acceleration=0,
        ...
        obstacle_3 = DynamicObstacle(3, ObstacleType.CAR, Rectangle(5, 2),
                                     ...
                                     TrajectoryPrediction(Trajectory(initial_time_step=4, state_list=[
                                         CustomState(time_step=4, position=np.array([20, 2]), velocity=10,
                                                     acceleration=0,
                                                     orientation=0)]), Rectangle(5, 2)))

        crcpp.register_scenario(123, "testScenario", 0, 0.1, "DEU", self.lanelet_network, [obstacle_1], [obstacle_2, obstacle_3])
