...
class TestAtRedLeftTrafficLightPredicates(unittest.TestCase):
    ...
    def test_at_red_left_traffic_light(self):
        # expected solutions
        ...
        obstacle_1 = DynamicObstacle(1, ObstacleType.CAR, Rectangle(5, 2),
                                     State(time_step=0, position=np.array([26.5, -15.0]), velocity=0, acceleration=0,
                                     ...
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         ...
                                         State(time_step=2, position=np.array([26.5, 1.5]), velocity=0,
                                               acceleration=0, orientation=math.pi / 2),
                                         State(time_step=3, position=np.array([26.5, 15.0]), velocity=0,
                                               acceleration=0, orientation=math.pi / 2)]), Rectangle(5, 2)))

        obstacle_2 = DynamicObstacle(2, ObstacleType.CAR, Rectangle(5, 2),
                                     State(time_step=0, position=np.array([40.0, 2.5]), velocity=0,
                                           acceleration=0, orientation=0),
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         State(time_step=1, position=np.array([30.0, 2.5]), velocity=0, acceleration=0,
                                               orientation=0)]), Rectangle(5, 2)))

        cpp_env_model.register_scenario(123, 0, "DEU", self.lanelet_network, [obstacle_1, obstacle_2], [])

        # Monitor-Mode
        sol_monitor_mode_1 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 0, 1)
        sol_monitor_mode_2 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 1, 1)
        sol_monitor_mode_3 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 2, 1)
        sol_monitor_mode_4 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 3, 1)
        sol_monitor_mode_5 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 0, 2)
        sol_monitor_mode_6 = cpp_env_model.at_red_left_traffic_light_boolean_evaluation(123, 1, 2)
