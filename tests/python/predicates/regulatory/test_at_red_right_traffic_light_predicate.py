...
class TestAtRedRightTrafficLightPredicates(unittest.TestCase):
    def setUp(self):
    ...
    def test_at_red_right_traffic_light(self):
        # expected solutions
        ...
        obstacle_1 = DynamicObstacle(1, ObstacleType.CAR, Rectangle(5, 2),
                                     State(time_step=0, position=np.array([15.0, 0.0]), velocity=0, acceleration=0,
                                           orientation=0),
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         State(time_step=1, position=np.array([20.0, 0.0]), velocity=0,
                                               acceleration=0, orientation=0),
                                         State(time_step=2, position=np.array([25.0, 0.0]), velocity=0,
                                               acceleration=0, orientation=0),
                                         State(time_step=3, position=np.array([40.0, 0.0]), velocity=0,
                                               acceleration=0, orientation=0)]), Rectangle(5, 2)))

        obstacle_2 = DynamicObstacle(2, ObstacleType.CAR, Rectangle(5, 2),
                                     State(time_step=0, position=np.array([26.5, -13.5]), velocity=0,
                                           acceleration=0, orientation=math.pi/2),
                                     TrajectoryPrediction(Trajectory(initial_time_step=1, state_list=[
                                         State(time_step=1, position=np.array([26.5, -3.5]), velocity=0, acceleration=0,
                                               orientation=math.pi/2)]), Rectangle(5, 2)))

        crcpp.register_scenario(123, 0, self.dt, "DEU", self.lanelet_network, [obstacle_1, obstacle_2], [])

        # Monitor-Mode
        sol_monitor_mode_1 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 0, 1)
        sol_monitor_mode_2 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 1, 1)
        sol_monitor_mode_3 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 2, 1)
        sol_monitor_mode_4 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 3, 1)
        sol_monitor_mode_5 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 0, 2)
        sol_monitor_mode_6 = crcpp.at_red_right_traffic_light_boolean_evaluation(123, 1, 2)
